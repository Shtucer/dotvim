...
if exists('loaded_winproject') || &cp
    finish
endif
let loaded_winproject=1

let s:project_app_name="none"
" <<< -- stuff used by winmanager
let g:ProjectExplorer_title = "[Project Explorer]"

function! ProjectExplorer_Start()
...
function! ProjectExplorer_WrapUp()
...
function! s:Project(filename) " <<<
    ...
    "    let b:proj_resize_command='silent! '
    ...
    function! s:DoSetupAndSplit()
        ...
        if s:project_app_name == "winmanager"
            "           echo "WinManager!"
            "           return
        endif
    ...
    function! s:SetupAutoCommand(cwd)
        if !exists("b:proj_has_autocommand")
            ...
            exec 'au BufEnter '.bufname." set filetype=project"
    ...
    function! s:GetScriptin(info, home)
        ...
        function! s:GetScriptout(info, home)
            ...
            " Mappings <<<
            nnoremap <buffer> <silent> <Return>   \|:call <SID>DoFoldOrOpenEntry('', 'wmgr')<CR>
            nnoremap <buffer> <silent> <S-Return> \|:call <SID>DoFoldOrOpenEntry('', 'wmgr')<CR>
        ...
        if !exists("*<SID>DoCloseProject()") "<<<
            function! s:DoCloseProject()
                if exists(g:proj_running)
                    Project
                    hide
                endif
        ...
        nnoremap <script> <Plug>CloseProject :call <SID>DoCloseProject()<CR>
        if exists('g:proj_flags') && (match(g:proj_flags, '\Cg') != -1)
            if !hasmapto('<Plug>ToggleProject')
                nmap <silent> <F12> <Plug>ToggleProject
            endif
            if !hasmapto('<Plug>CloseProject')
                nmap <silent> <F11> <Plug>CloseProject
            endif
        endif
